...
int SHA1Result(SHA1Context *context, uint8_t Message_Digest[SHA1HashSize])
{
	...
	for(i = 0; i < SHA1HashSize; ++i) Message_Digest[i] = context->Intermediate_Hash[i >> 2] >> 8 * (3 - (i & 0x03));
 ...
 *
 */
int SHA1Input(SHA1Context *context, uint8_t *message_array, unsigned length)
{
...
int SHA1(uint8_t *message_array, unsigned int length, uint8_t Message_Digest[SHA1HashSize])
{
	...
	if (ret == shaSuccess)
	{
		ret = SHA1Input(&ctx, message_array, length);
		if (ret == shaSuccess) ret = SHA1Result(&ctx, Message_Digest);
	}
